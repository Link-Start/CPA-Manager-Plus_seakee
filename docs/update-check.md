...
Manager Server 负责检查 CPAMP 更新。面板读取服务端缓存；手动检查有服务端 60 秒冷却。后台首次延迟 5–59 秒，此后约每 6 小时检查一次；失败后退避 15–30 分钟。设置环境变量 `CPAMP_UPDATE_CHECK_ENABLED=false` 关闭后台检查，手动检查仍可用。
