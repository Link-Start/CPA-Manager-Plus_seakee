...
- `POST /v0/management/usage/archives/{id}/cancel` abandons a run before raw deletion starts. Published archive files and the identity ledger remain; raw usage is not deleted. `previewed`, `archived`, `verified`, and `failed` runs with no published segment or delete activity can be cancelled. `archiving`, `verifying`, `deleting`, `failed` runs with a published segment or partial deletion, and `completed` runs cannot.
